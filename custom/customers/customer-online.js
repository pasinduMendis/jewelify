...
import Loading from "../loadingPages/customerLoading";
...
export default function Online({invenData,addInv}) {
  ...
  const fetchData = async () => {
    setTableLoding(true);
      await axios
        .get(`https://api.jewelify.ai/.netlify/functions/customers?type=online` , {
          headers: {
      ...
      if (inven.data.orders.length > 0) {
        ...
        for (let i = 0; i < inven.data.orders.length; i++) {
          ...
          if (inven.data.orders[i].order_status != "" && inven.data.orders[i].order_status == "processing") {
            processingCount = processingCount + 1;
          }
          if (inven.data.orders[i].order_status != "" && inven.data.orders[i].order_status == "completed") {
            completedCount = completedCount + 1;
          }
          if (inven.data.orders[i].order_status != "" && inven.data.orders[i].order_status == "on-hold") {
  ...
  const fetchDataFiterOffline = async () => {
    ...
    session &&
      (await axios
        .get(`https://api.jewelify.ai/.netlify/functions/customers?type=online&keyWord=${filterData.keyWord}&sku=${filterData.sku}&customerId=${filterData.customerId}&email=${filterData.email}` , {
          headers: {
            Authorization: session.authToken,
          },
        })
     ...
     marginTop: "0px"}}>
            ...
            <div className="w-embed" >
                  ...
                  <div className="table-wrap" style={{zIndex:2}}> 
                      <table className="table-1" style={{width:'auto',minWidth:"100%",}}>
